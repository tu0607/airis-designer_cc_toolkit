...
const DOCS = ['CLAUDE.md', 'README.md',
  ...
  'rules/README.md', 'rules/flow.md', 'rules/principles.md', 'rules/common.md',
...
const RUNTIME_PATHS = new Set(['config/sd.config.js'])
...
const allSec = new Set(Object.values(sec).flatMap((s) => [...s]))
{
  ...
  for (const f of DOCS) {
    const s = read(f)
    for (const m of s.matchAll(/`?(flow\.md|common\.md|web-app(?:-styling|-storybook|-testing|-ci|-selfcheck)?\.md|web-lp\.md|web-content-site\.md|native\.md|handoff\.md)`?\s*(?:の)?\s*(?:ステップ\s*|§)([0-9]+(?:[-.][0-9]+)*)/g)) {
      const k = 'rules/' + m[1]
    ...
    const clHeads = Object.fromEntries([...read('rules/flow.md').matchAll(/^### (\d+)\.\s+(.*)$/gm)]
...
// ── E. リンク・参照パスの実在 ────────────────────────────────────
{
  ...
  for (const f of DOCS) {
    ...
    for (const m of s.matchAll(/`((?:scripts|rules|config|\.claude)\/[A-Za-z0-9_./-]+)`/g)) {
      const t = m[1]
      if (t.includes('<') || t.includes('*') || RUNTIME_PATHS.has(t)) continue
...
{
  const iss = []
  for (const f of ['config/project.example.json', '.claude/settings.json',
