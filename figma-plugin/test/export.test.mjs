...
// ============ ⑨ アイコン / ロゴの書き出し ============
// **どちらもエラーを出さずに結果が変わる**種類の不具合の再発防止。
//   - バリアント付きアイコンが 1 件も出ない（書き出しが 0 件になり icons.json が消える）
//   - "logout" が "logo" の部分一致でロゴ扱いされる
// assets/ には前回の書き出しが残るので、**差分も CI も出ない**まま気付けない。
console.log('\n=== ⑨ アイコン / ロゴの書き出し ===');
const results3 = [];
const p1 = pass, f1 = fail;
{
  const t = (name, cond, detail = '') => {
    if (cond) { pass++; results3.push(`  ✅ ${name}`); }
    else { fail++; results3.push(`  ❌ ${name}${detail ? ' … ' + detail : ''}`); }
  };

  const svg = (marker) =>
    new TextEncoder().encode(
      `<svg width="24" height="24" viewBox="0 0 24 24"><path d="${marker}" fill="#111111"/></svg>`,
    );
  // アイコン/ロゴのノード。exportAsync を生やす（本物の Figma が持つ API）
  const art = (name, type, marker) => {
    const n = node(name, type, {}, []);
    n.exportAsync = async () => svg(marker);
    return n;
  };

  // (a) 単体 COMPONENT のアイコン（従来の形）
  const plain = art('Icons/check', 'COMPONENT', 'M-CHECK');

  // (b) バリアント付きアイコン。**既定バリアントを先頭以外に置く**ことで
  //     「子の並び順で拾っていないか」を判別できるようにする
  const starFilled = art('fill=Filled', 'COMPONENT', 'M-STAR-FILLED');
  starFilled.variantProperties = { fill: 'Filled' };
  const starOutlined = art('fill=Outlined', 'COMPONENT', 'M-STAR-OUTLINED');
  starOutlined.variantProperties = { fill: 'Outlined' };
  const starSet = {
    id: '700:1', name: 'Icons/star', type: 'COMPONENT_SET',
    componentPropertyDefinitions: {},
    children: [starFilled, starOutlined],
    defaultVariant: starOutlined, // ← 先頭ではない
  };

  // (c) "logout" は "logo" を含む。ロゴとして書き出されてはいけない
  const logoutV = art('style=Line', 'COMPONENT', 'M-LOGOUT');
  logoutV.variantProperties = { style: 'Line' };
  const logoutSet = {
    id: '700:2', name: 'Icons/logout-door', type: 'COMPONENT_SET',
    componentPropertyDefinitions: {},
    children: [logoutV], defaultVariant: logoutV,
  };

  // (d) 本物のロゴ。カラー variant が全部出ること
  const logoDefault = art('Color=Default', 'COMPONENT', 'M-LOGO-DEFAULT');
  logoDefault.variantProperties = { Color: 'Default' };
  const logoWhite = art('Color=White', 'COMPONENT', 'M-LOGO-WHITE');
  logoWhite.variantProperties = { Color: 'White' };
  const logoSet = {
    id: '700:3', name: 'Logo/Brand', type: 'COMPONENT_SET',
    componentPropertyDefinitions: {},
    children: [logoDefault, logoWhite], defaultVariant: logoDefault,
  };

  // 本物の findAllWithCriteria({types:['COMPONENT']}) は **セットの子も返す**ので、
  // スタブでも同じにする（子の名前は "fill=Filled" なので ICON_PREFIX に一致しない）
  const { posted: posted3 } = run({
    components: [plain, starFilled, starOutlined, logoutV, logoDefault, logoWhite],
    componentSets: [starSet, logoutSet, logoSet],
  });
  await new Promise((r) => setTimeout(r, 50));

  const m3 = posted3.find((x) => x.type === 'tokens');
  const e3 = posted3.find((x) => x.type === 'error');
  if (e3) {
    t('buildBundle が例外を出さない', false, e3.message);
  } else if (!m3) {
    t('buildBundle が結果を返す', false, '何も返らなかった');
  } else {
    const icons = m3.bundle['icons.json'] || {};
    const logos = m3.bundle['logos.json'] || {};
    const meta3 = m3.bundle['$meta.json'];

    t('単体 COMPONENT のアイコンが書き出される（既存の挙動）',
      typeof icons.check === 'string' && icons.check.indexOf('M-CHECK') !== -1,
      JSON.stringify(Object.keys(icons)));

    t('★ バリアント付きアイコンが書き出される（0 件になる不具合の再発防止）',
      typeof icons.star === 'string',
      JSON.stringify(Object.keys(icons)));

    t('★ 書き出されるのは既定バリアント（並び順ではない）',
      typeof icons.star === 'string' && icons.star.indexOf('M-STAR-OUTLINED') !== -1,
      icons.star ? icons.star.slice(0, 90) : String(icons.star));

    t('★ Icons/logout-* がロゴとして書き出されない（logo の部分一致）',
      Object.keys(logos).every((k) => k.indexOf('logout') === -1),
      JSON.stringify(Object.keys(logos)));

    t('logout はアイコン側に出る',
      typeof icons['logout-door'] === 'string',
      JSON.stringify(Object.keys(icons)));

    t('ロゴは全カラー variant が書き出される',
      !!logos['logo-brand'] &&
        typeof logos['logo-brand'].default === 'string' &&
        typeof logos['logo-brand'].white === 'string',
      JSON.stringify(logos['logo-brand'] && Object.keys(logos['logo-brand'])));

    t('$meta.icons.count が実数と合う',
      meta3.icons.count === Object.keys(icons).length,
      `meta=${meta3.icons.count} / 実数=${Object.keys(icons).length}`);

    t('書き出しの失敗が記録されていない',
      meta3.validation.exportFailures.length === 0,
      JSON.stringify(meta3.validation.exportFailures));
  }
}
console.log(results3.join('\n'));
console.log(`\n⑨ アイコン / ロゴ: ${pass - p1} 件成功 / ${fail - f1} 件失敗`);
