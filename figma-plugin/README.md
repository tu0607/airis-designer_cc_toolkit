...
[`../rules/figma-plugin-airis.md`](../rules/figma-plugin-airis.md)、その先のコミット・PR は [`../rules/flow.md`](../rules/flow.md) が正。
...
- **Components は全件書き出し**：既定で全 ComponentSet/Component（`COMPONENT_ALLOWLIST` 空）。
  ...
  **`bindings` はトークン束縛だけ**を持つ（レイアウトの実数値は別キー `layout` に出す。上記）。
  ...
  - **INSTANCE の内部には再帰しない**（アイコン等は別コンポーネントの領域）。
    ボタン自身が持つ束縛のみを契約に含める。
- **アイコン / ロゴは名前で拾うので、命名を外すと静かに 0 件になる**:
  - アイコン = 名前が `Icons/` で始まる COMPONENT または COMPONENT_SET（セットは**既定バリアント**を書き出す）。
  - ロゴ = 名前に**単語としての `logo` / `logos`** を含む COMPONENT_SET。部分一致にすると `logout` を拾ってしまうため単語境界で見ている。
    **その裏返しで `logotype` は対象外**になる（`logo` に続けて英字があるため）。ロゴは `Logo/` を含む名前で作る。
  - **0 件のとき `icons.json` / `logos.json` は bundle ごと出ない**（`validation` にも残らない）。
    取り込み側は `$meta.json` の `icons.count` / `logos.count` を見て、**前回より減っていないか**を確認する。
    `assets/` には前回の書き出しが残るので、**差分も出ず CI も緑のまま**気付けない。
