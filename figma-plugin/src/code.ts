...
async function buildIcons(): Promise<Dict> {
  ...
  for (const node of nodes) {
    try {
      ...
      const name = slug(node.name.slice(node.name.indexOf('/') + 1));
      if (!name) {
        validation.exportFailures.push(`icon "${node.name}": 名前が空になった`);
        continue;
      }
      if (out[name] !== undefined) {
        validation.exportFailures.push(`icon "${node.name}": 名前 "${name}" が重複`);
        continue;
      }
      out[name] = normalizeIconSvg(utf8Decode(bytes));
    } catch (e) {
      validation.exportFailures.push(`icon "${node.name}": ${String(e)}`);
    }
  }
  return out;
}
...
async function buildLogos(): Promise<Dict> {
  ...
  const sets = figma.root
    ...
    .filter((s) => s.name.toLowerCase().indexOf('logo') !== -1);
