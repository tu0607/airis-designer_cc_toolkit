...
│   ├── components-diff.mjs       # ▶ 部品の定義が変わったかを前後の components.json で比べる
│   │                             #   （トークン更新だけの運用で、バリアント追加を見逃さないため）
...
インストールと同じ 1 行を実行すれば最新版になります（何度実行しても安全）。

```bash
curl -fsSL https://raw.githubusercontent.com/tu0607/airis-designer_cc_toolkit/main/install.sh | sh
```

- **入るのは中身だけ**（tarball を展開するので `.git` は作りません）。
- **あなたが編集したファイルは上書きしません。** 取得時のハッシュを `.airis-manifest` に記録しているので、`rules/` を自社向けに直していれば**スキップして名前を報告**します。最新版を取り込みたいときは、そのファイルを退避してから再実行してください。
...
- 手動でも入れられます（Code → Download ZIP → 展開 → `npm install`）。ただし**手動展開した場所はこのコマンドで更新できません**（判断に使う `.airis-manifest` が無いため）。継続的に更新するならインストーラーを使ってください。
