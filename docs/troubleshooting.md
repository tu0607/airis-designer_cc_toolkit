...
| 症状 | 対処 |
| --- | --- |
| Figma のデータが取れない | Dev Mode は対象を選択＆ローカル MCP 有効化を確認。Framelink は `/airis:setup` でトークン再設定。 |
| `FIGMA_API_KEY` が反映されない | `/airis:setup` を再実行し、Claude Code を起動し直す（`.claude/settings.local.json` の env はセッション開始時に読み込まれる）。 |
