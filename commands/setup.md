---
description: 初回セットアップ（前提確認・.airis/ の用意・任意の Figma 接続・GitHub 公開設定の案内）。冪等。
---
...
### 6. Storybook の共有と自動テストの準備（GitHub。全員）
...
#### 6-1. 誰が見られるようになるかを**調べて**から伝える（**先に必ず実施する**）
...
**① 調べる**（対象リポジトリ未定なら「Push のときに改めて確認します」と伝えて 6-2 へ）
...
**② 判定して、そのまま伝える**（判定表は `<Airis>/rules/web-app-storybook.md` §2）
...
- **判断がつかない / 対象未定** → 保留（`visibilityAcknowledged` は `false` のまま。`/airis:build-code` の Push 時に再確認）。
...
#### 6-3. GitHub Pages を有効にする手順を案内する
...
- **設定が終わったか**をこの場で確認する。終わっていれば `storybook.pagesEnabled` を `true` に、まだなら `false` のままにする（`false` の間は `/airis:build-code` の Push のたびに手順を再掲する）。
