...
0. **社内プラグインの取り込みルールが 1 つ用意されている**ので、**一度だけ触れる**（押しつけない）:
   ...
   - **使う** → `tokens.customPluginSpec` を `<Airis>/rules/figma-plugin-airis.md` に設定する。**ビルドはこのコマンドでは行わない**（`/airis:build-plugin` の仕事）:
     - プロジェクト直下の `figma-plugin/manifest.json` が**既にある**（誰かがビルドしてコミット済み）→ その絶対パスを伝え、Figma への読み込み（Plugins → Development → Import plugin from manifest…）を案内する。ビルド不要。
     - **無い** → 「このあと `/airis:build-plugin` を実行してください（依存の導入からビルドまで自動で、成果物はプロジェクト直下の `figma-plugin/` に置かれます。コミットすればチームで共有できます）」と案内し、**「残っている作業」に記録**する。`<Airis>` 配下にビルドしない。
     - 下の 1〜3 は不要。
...
- `.airis/`（`config.json`・`rules/`）と、同梱プラグインを使うならプロジェクト直下の `figma-plugin/` を**コミットして共有する**ことを案内する（コミット自体はこの場で提案してよい。Push はしない）。
- Figma トークンを新規設定した場合は「Claude Code の再起動が必要」を再掲。
- **未完了の項目を「残っている作業」として明示**する（例: `gh auth login`・`/airis:build-plugin`。誰がやるか・どこでやるかを 1 行ずつ）。
