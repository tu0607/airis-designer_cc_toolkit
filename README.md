...
ターミナルで 1 行実行すると、前提チェック〜取得〜依存の導入まで自動で行います（**何度実行しても安全**。2 回目以降は最新版への更新になります）:
...
> - **入るのは中身だけです**（tarball を展開するので `.git` は作りません）。Airis 自体の履歴は不要なので持ち込みません。
> - **更新しても、あなたが編集したファイルは上書きされません。** 取得時のハッシュを `.airis-manifest` に記録しているので、`rules/` を自社向けに直していれば**スキップして名前を報告**します（最新版を取り込みたいときは退避してから再実行してください）。
> - **上流で削除されたファイルは片付けます**（古いルールが残ると Claude がそれを読んでしまうため）。ただし**編集していたものは消さずに報告**します。あなたが自分で足したファイルには触りません。
> - **更新後は [`CHANGELOG.md`](CHANGELOG.md) を見てください。** ルールファイルのリネームなど、**あなたが自分で書いた参照**を直す必要がある変更だけが載っています（台帳は Airis の写しは守りますが、あなたの記述までは追えません）。
> - リポジトリが非公開の場合は `gh auth login` を済ませてから実行してください（`gh` 経由で取得します）。
> - 手動でも入れられます（Code → Download ZIP → 展開 → `npm install`）。ただし**手動展開した場所は上の 1 行では更新できません**（更新の判断に使う `.airis-manifest` が無いため）。継続的に更新するなら 1 行のインストーラーを使ってください。
