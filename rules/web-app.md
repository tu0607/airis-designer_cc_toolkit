...
生成物は **Push 先リポジトリの作業ツリーへ直接書く**（`.airis/work/` へ書いてから配置し直す二段構えはしない。`flow.md` ステップ 8）。
...
```
<リポジトリのルート>/
├── .airis/                         # Airis の設定（`/airis:setup` が生成。config.json / rules/ 差分 / work/）
│                                   #   受け取った人は Airis プラグインを入れてここで起動する（`handoff.md` §1）
...
**(2) 配置先ディレクトリの中**（`flow.md` ステップ 5 で確認した `src/` 等。ルート直下でも構わない）
...
> **配置先が `src/` の場合、`sd.config.js` の出力先と各ワークフローの `styles/tokens.css` は `src/styles/tokens.css` に置換する**（`flow.md` ステップ 8）。雛形はルート直下の前提で書いてあるので、ここを直さないと「トークンが反映されているか確認」ステップが必ず落ちる。
>
...
- **Push 前にローカルでテスト・ビルド・Storybook を実行して「検証済み」とすること**（検査は GitHub Actions の役目。`web-app-storybook.md` §2・`principles.md` 原則 2）。
...
- **PR 本文に「受け取った人の始め方」（Airis プラグインの導入 + `/airis:setup`）を書かずに引き渡すこと**（受け取った人の環境では原則が 1 つも読み込まれない。`handoff.md` §1）。
