...
- プラグインの所在: **このリポジトリの `figma-plugin/`**（Airis が管理する。実装・出力仕様の詳細は `<Airis>/figma-plugin/README.md`）
...
> **このプラグインを使うかどうかは操作者の判断。** 合わなければ `tokens-studio` / `plugin` / `claude` でも成立する（`flow.md` ステップ 6-1）。
...
**各キーの意味は `<Airis>/figma-plugin/README.md`（出力の契約）が正。** ここでは診断としての扱いだけを決める。
...
> **`schemaVersion` が想定と違うとき、推測で読み進めない。** `<Airis>/figma-plugin/README.md` の「版の履歴」で差分を確認し、このファイルを更新してから進める（**両方このリポジトリにあるので同時に直せる**）。
