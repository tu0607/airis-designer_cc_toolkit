...
| キー | 使い方 |
| --- | --- |
| `schemaVersion` | **このルールが想定するのは `4`。** 一致しなければ止める。`4` 未満なら**互換対応せず、`sh figma-plugin/setup.sh` の後に Figma で書き出し直してもらう**。`4` より新しければ `figma-plugin/README.md` の「版の履歴」を確認してこのファイルを更新する |
...
> **`schemaVersion` が想定と違うとき、推測で読み進めない。** `figma-plugin/README.md` の「版の履歴」で差分を確認し、このファイルを更新してから進める（**両方このリポジトリにあるので同時に直せる**）。
