...
| ファイル           | 役割                                                        |
| -------------- | --------------------------------------------------------- |
| `flow.md`      | **フロー全体の「正」**（何を、どの順で、誰に確認するか。`/airis:build-code` `/airis:build-token` の手順の実体） |
| `principles.md` | 絶対に守る 4 原則（`.airis/` のあるプロジェクトではセッション開始時に自動注入される） |
...
| `handoff.md`   | Push 先リポジトリの引き継ぎと git 操作（受け取った人への案内 / push が拒否されるとき / マージ後の片付け） |
...
- **利用者**は各ファイルを直接編集せず、自分のプロジェクトの `.airis/rules/` に**同名ファイルで差分だけ**を置いてください（プラグインの更新で上書きされないため）。ここを編集するのは Airis 自体のメンテナ（またはフォーク運用者）です。
- 会社/プロダクトのコーディング規約、コンポーネントライブラリ、デザインシステムに合わせてカスタマイズできます。
- 新しいターゲット（例: Web の別フレームワーク）を追加する場合:
  1. `rules/<target>.md` を追加。
  2. `flow.md` のフロー「3. プラットフォーム / ターゲット判定」「4. 変換ルールの読み込み」に対象を追記。
