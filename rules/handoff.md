...
| `.claude/commands/*.md` | そのリポジトリ向けの入口（例: トークンを取り込んで再生成する / セルフチェックを通す）。**トークン更新のコマンドを作るなら下記の検査を必ず組み込む** |
